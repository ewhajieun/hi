...
mypremium<-function(kilometres,zone,bonus,make,limit,deductable){
  ...
  if(make=="VOLVO V60 N MILJODIESEL Diesel"){make<-1
  }else if(make=="VOLVO V40N CC MILJODIESE Diesel"){make<-2
  }else if(make=="VOLVO S60 II MILJODIESEL Diesel"){make<-3
  }else if(make=="VOLVO V40N MILJODIESEL Diesel"){make<-4
  }else if(make=="VOLVO XC60 DRIVE-E Diesel"){make<-5
  }else if(make=="VOLVO V60CC MILJODIESEL"){make<-6
  }else if(make=="VOLVO XC70II DRIVE-E Diesel"){make<-7
  }else if(make=="VOLVO V60 N LADDHYBRID Laddhybrid"){make<-8
  }else if(make=="VOLVO V60CC MILJODIESEL"){make<-9}
  ...
  beta4<-0
  if(zone=="Rural areas in southern Sweden"){beta4<-1}else if(zone=="Gotland"){beta4<-1}

  beta5<-ifelse(zone=="Rural areas in northern Sweden",1,0)
  beta6<-ifelse(zone=="Smaller cities with surroundings in northern Sweden",1,0)
  ...
  s1<-0
  s7<-0
  if(make==8){s1<-1}else if(make==1){s1<-1}
  if(make==9){s7<-1}else if(make==9){s7<-1}
...
alpha1_1<-ifelse(kilometres=="less than 1000",1,0)
  alpha2_1<-ifelse(kilometres=="from 1000 to 15000",1,0)
  alpha3_1<-ifelse(kilometres=="15000 to 20000",1,0)
  alpha4_1<-ifelse(kilometres=="20000 to 25000",1,0)
  alpha5_1<-ifelse(kilometres=="more than 25000",1,0)
  
  beta1_1<-ifelse(zone=="Stockholm, Goteborg, Malmo with surroundings",1,0)
  beta2_1<-ifelse(zone=="Other large cities with surroundings",1,0)
  beta3_1<-ifelse(zone=="Smaller cities with surroundings in southern Sweden",1,0)
  beta4_1<-ifelse(zone=="Rural areas in southern Sweden",1,0)
  beta5_1<-ifelse(zone=="Smaller cities with surroundings in northern Sweden",1,0)
  beta6_1<-ifelse(zone=="Rural areas in northern Sweden",1,0)
  beta7_1<-ifelse(zone=="Gotland",1,0)
 ...
 coef.data1<-c(-2.9177,-0.5704,-0.4144,-0.2873,-0.2068,0.3189,0.0798,-0.0679,-0.2557,-0.2054,
           1.0697,0.7859,0.6552,0.5067,0.4067,0.3086,0.007,0.1522,-0.1606,-0.5656,0.1640,-0.2619,-0.0684,
          0.0932,0.2752,0.2436,0.2501,0.3146,0.1379,0.2143,-0.1055,0.3098,0.0905,0.1530,0.1023)
 ...
 coef.data2<-c(8.476,3.080,-8.337,7.659,-1.237,2.287,4.785,1.287,5.170,1.465,2.278,6.917,4.979,5.119,-7.706,-1.821,-5.207,-3.523,8.435,-1.643,-8.718,-3.932,-1.194,2.135,-5.490 )
  ...
  if (size.claim<deductable){
    size.claim1<-0}else if(size.claim>=deductable& size.claim<limit){
      size.claim1<-size.claim-deductable
    }else if(size.claim>=limit){
      size.claim1<-limit-deductable
    }
  premium<-c(n.claim*size.claim1,"Krona",n.claim*size.claim1*100/22,"USD")
  return(premium)
}
  ...
  sidebarPanel(  
   ...
   selectInput(inputId="make",
               ...
               choices=choices=c("VOLVO V60 N MILJODIESEL Diesel",
                          ...
                          "VOLVO XC60 DRIVE-E Diesel",
                          "VOLVO V60CC MILJODIESEL",
                          "VOLVO XC70II DRIVE-E Diesel",
                          "VOLVO V60 N LADDHYBRID Laddhybrid",
                          "VOLVO V60CC MILJODIESEL"),
               selected=1),
   numericInput("limit",
